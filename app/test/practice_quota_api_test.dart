...
void main() {
  ...
  test('createAdRewardSession sends bearer token and maps SSV token', () async {
    Uri? requestedUri;
      ...
      client: ApiClient(
        ...
        postJsonWithHeadersTransport: (uri, body, timeout, headers) async {
          ...
          return ApiResponse(
            statusCode: 200,
            body: jsonEncode({
              'sessionToken': 'ssv-session-token',
              'expiresAt': '2026-06-17T12:32:42+09:00',
            }),
          );
        },
      ),
    ...
    final session = await api.createAdRewardSession(
      accessToken: ' access.jwt ',
    );

    expect(
      requestedUri.toString(),
      'http://localhost:8080/api/quota/ad-reward-sessions',
    );
    expect(requestedHeaders, {'Authorization': 'Bearer access.jwt'});
    expect(requestedBody, isEmpty);
    expect(session.sessionToken, 'ssv-session-token');
    expect(session.expiresAt, DateTime.parse('2026-06-17T12:32:42+09:00'));
  });

  test('fetchAdRewardSessionStatus maps verified server state', () async {
    Uri? requestedUri;
    Map<String, String>? requestedHeaders;
    final api = DartIoPracticeQuotaApi(
      client: ApiClient(
        ...
        getJsonTransport: (uri, timeout, headers) async {
          ...
          return ApiResponse(
            statusCode: 200,
            body: jsonEncode({'status': 'COMPLETED', 'credited': true}),
          );
        },
      ),
    );

    final status = await api.fetchAdRewardSessionStatus(
      accessToken: 'access.jwt',
    ...
    expect(
      requestedUri.toString(),
      'http://localhost:8080/api/quota/ad-reward-sessions/session%2Ftoken',
    );
    expect(requestedHeaders, {'Authorization': 'Bearer access.jwt'});
    expect(status.status, AdRewardStatus.completed);
