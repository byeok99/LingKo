...
## 2026-08-04 - 로컬라이제이션 회귀 테스트

- 변경 파일: `localization_test.dart`, `widget_test.dart`
- 내용: 설정에서 고를 수 있는 언어가 모두 지원 목록에 있는지, 번역이 없는 로케일에서도 문자열이 비지 않는지 고정했다. 위젯 테스트도 앱과 같은 로컬라이제이션 경계를 갖도록 래퍼를 추가했다.
