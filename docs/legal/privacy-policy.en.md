# LingKo Privacy Policy

> **All values in this document are confirmed.** No placeholders remain. It has not been reviewed by a lawyer, and whether Azure Speech uses recordings for model training is unverified. See [README.md](./README.md) for details.

LEE SANG BYEOK ("we", "us", "our") explains below how we handle personal data when you use LingKo (the "Service"). This Policy applies to users in all regions. Additional rights available depending on where you live are set out in Section 12.

The Service is operated by an **individual**, not a company. We keep no business premises or telephone line and receive every enquiry at a single email address.

- Effective Date 2026-09-04
- Last Updated 2026-09-04

---

## 1. Purposes of Processing

| Purpose | Detail |
|---|---|
| Account registration and management | Identifying and authenticating you via your linked Google or Apple account, maintaining login sessions, preventing duplicate accounts, processing account deletion |
| Providing pronunciation assessment | Analysing your recording to produce a pronunciation score, the recognised sentence, syllable-level scores, and weak characters |
| Providing learning history | Storing and displaying your practice history, saved sentences, and past assessment results |
| Managing evaluation credits | Calculating your remaining credits and recharge timing |
| Providing learning guides | Generating and delivering guide videos that support pronunciation practice |
| Serving advertising | Displaying ads in the Service, measuring ad performance, preventing click fraud |
| Marketing communications | Sending news about events, benefits, and new features (only to users who opt in) |
| Improving the Service | Analysing usage and error patterns to improve quality and respond to incidents |
| Handling enquiries | Receiving, processing, and answering enquiries and complaints |

If the purpose of processing changes, we will take the steps required by applicable law, including obtaining separate consent where necessary.

---

## 2. Personal Data We Process

### Required

| Data | Purpose |
|---|---|
| Social account identifier and provider (Google, Apple) | Identifying you, preventing duplicate accounts |
| Email address | Identifying you, replying to enquiries, sending important notices |
| Name, profile image URL | Displaying you within the Service |
| Voice recordings you make (WAV) | Performing the pronunciation assessment |
| Assessment results (overall score, recognised sentence, syllable-level scores, weak characters) | Returning results, providing learning history |
| Saved sentences, practice history | Providing learning history and repeat practice |
| Evaluation credit balance and recharge timestamps | Managing your usage allowance |

If you sign in with Apple and choose Hide My Email, we receive and process Apple's private relay address rather than your real email address.

### Optional

| Data | Purpose | If you decline |
|---|---|---|
| Marketing consent status and consent/withdrawal timestamps | Sending news about events, benefits, and new features | No restriction on your use of the Service; you simply do not receive marketing messages |

### Collected automatically

- IP address, device information (OS type and version, app version, device model)
- Service usage records, API request records, access timestamps
- Login session identifiers and the hash of your refresh token
- **Advertising identifiers (Android Advertising ID, iOS IDFA)** and ad impression/click records — see Section 6

### Sensitive data

We do not collect sensitive data or government-issued identification numbers.

Voice recordings are processed **solely to measure the accuracy of Korean pronunciation**, never to identify you. We do not subject recordings to any technical processing intended to uniquely identify a natural person, so we do not create biometric information from them.

---

## 3. How We Collect Data

- From Google or Apple during social sign-in, limited to the scopes you approve
- Directly from you, when you record and upload audio in the app
- Automatically, as usage records and advertising-related data generated while you use the Service
- From your marketing consent selection at sign-up
- From the content you provide when contacting us

---

## 4. Retention

| Data | Retention |
|---|---|
| Account data | Deleted without undue delay when you close your account. If you do not close it, the account becomes dormant after 12 months without a login and is deleted 6 months after that |
| **Voice recordings you upload** | **Kept only for the minimum period needed to perform the assessment, and deleted without undue delay once the assessment succeeds or finally fails.** Files uploaded but never submitted, or whose individual deletion failed, expire one day after upload under a storage lifecycle policy |
| Assessment results, practice history, saved sentences | Deleted on account closure or with the dormant-account schedule above |
| Evaluation credits and recharge records | Deleted on account closure or with the dormant-account schedule above |
| Session data | Deleted on logout, session expiry, or account closure, whichever is earliest |
| Marketing consent and withdrawal records | 3 years after withdrawal, to evidence consent in the event of a dispute |
| Usage and access logs | 3 months |
| Advertising identifiers and ad interaction records | Not retained separately by us; Google's retention follows Google's policies |

Storage lifecycle expiry is processed asynchronously, so we cannot guarantee deletion at exactly 24 hours.

**Dormant accounts.** After 12 months without a login we place the account in a dormant state and store its personal data separately from that of active users. We email you at least 30 days before the account becomes dormant, and again at least 30 days before deletion. Signing in again lifts the dormant state immediately. We apply this so that we do not keep personal data for longer than is necessary for the purposes described here.

---

## 5. Sharing and Processors

### Third-party disclosure

We do not sell your personal data and do not disclose it to third parties except with your consent or as required by law.

As described in Section 6, advertising identifiers and related data are transmitted to Google for ad serving. Depending on your region, this may be treated as a disclosure, a sharing for cross-context behavioural advertising, or a transfer to a processor.

### Processors

| Processor | Service | Data processed | Location |
|---|---|---|---|
| Amazon Web Services, Inc. | Temporary storage of audio, infrastructure | Voice recordings | Republic of Korea (ap-northeast-2, Seoul) |
| Microsoft Corporation (Azure AI Speech) | Pronunciation assessment | Voice recordings, target sentence text | Republic of Korea (koreacentral) |
| Replicate, Inc. | Learning guide video generation | Sentence text and generation media; no data identifying you | United States |
| Google LLC (AdMob) | Ad serving and performance measurement | Advertising identifier, IP address, device information, ad interaction records | United States |

We bind each processor by contract to process data only on our instructions, apply technical and organisational safeguards, restrict sub-processing, and accept liability, and we supervise their compliance. We will publish any change of processor in this Policy.

Google LLC and Apple Inc. act as independent controllers in respect of your Google or Apple account and process your data under their own policies.

### International transfers

We are established in the Republic of Korea, so data is transferred outside the European Economic Area.

| Destination | Recipient | Safeguard |
|---|---|---|
| Republic of Korea | LEE SANG BYEOK, AWS ap-northeast-2 (Seoul) | Operator and primary storage are located in Korea. Korean law (PIPA) applies |
| United States | Google LLC, Replicate, Inc. | **Standard Contractual Clauses** under Art. 46(2)(c) — Commission Implementing Decision (EU) 2021/914, with a transfer impact assessment |

A copy of the Standard Contractual Clauses is available on request at maplebyeok@gmail.com.

You may object to international transfers. For advertising, you can limit them through the CMP and your device operating system's advertising and tracking settings; for anything else, contact us using Section 11.

---

## 6. Advertising

We fund the Service by serving ads through **Google AdMob**.

| Item | Detail |
|---|---|
| Collected by | Google LLC (AdMob) |
| Purpose | Ad serving, ad performance measurement, click-fraud prevention, personalised advertising |
| Data | Advertising identifier (Android Advertising ID, iOS IDFA), IP address, device information, ad impression and click records |
| Location | United States |
| Retention | Per Google's policies |

For details of Google's advertising practices see the Google Privacy Policy (https://policies.google.com/privacy) and Google's advertising partner policies.

### Opting out of personalised ads

You can opt out of personalised advertising at any time. Ads will still appear, but they will not be tailored to your interests.

- **iOS**: Settings › Privacy & Security › Tracking — turn off "Allow Apps to Request to Track"
- **Android**: Settings › Privacy › Ads — delete or reset your advertising ID

### Regional consent for advertising

- **Where local law requires prior consent**: on first launch, a Consent Management Platform (CMP) lets you choose **in advance** whether your data may be used for advertising purposes. You can use the Service without consenting; you will simply receive non-personalised ads.
- **iOS**: turning off Settings › Privacy & Security › Tracking stops the advertising identifier from being collected.
- **Elsewhere**: use your device operating system's advertising and tracking settings to limit identifiers used for personalised advertising.

### Cookies

LingKo is a mobile application and does not use web cookies. Your login session is held in the device's secure storage (iOS Keychain, Android Keystore) and is removed when you log out or uninstall the app.

---

## 7. Marketing Communications

If you **opt in** at sign-up, we send news about events, benefits, and new features by email or push notification.

- Marketing consent is **optional**. Declining places no restriction on your use of the Service.
- You may withdraw consent at any time via the unsubscribe link in any marketing email or using the contact details in Section 11.
- Even after withdrawal, we continue to send operational messages such as contract, security, and policy notices.

---

## 8. Deletion

When the retention period ends or the purpose is achieved, we delete the data without undue delay.

**Process.** When you request account deletion, we first delete your stored audio objects (including previous versions and delete markers). Only once that succeeds do we delete, in a single transaction, your login sessions, assessment jobs and results, evaluation credits, and account data. If the storage deletion fails, we preserve your data, stop the deletion, and let you retry.

**Method.** Electronic files are erased irrecoverably; paper records are shredded or incinerated.

Shared reference data such as the syllable dataset used as the pronunciation baseline does not relate to any identified individual and is therefore not personal data subject to deletion.

---

## 9. Your Rights and How to Exercise Them

You may at any time:

1. Request access to your personal data
2. Request rectification of inaccurate data
3. Request erasure
4. Request restriction of processing
5. Request portability of your data
6. Change or withdraw your marketing consent

**How**

- **In the app** — Settings gives you Terms of Service, Privacy Policy, and Account deletion (which erases everything)
- Email maplebyeok@gmail.com

The Service is run by an individual with no business premises or telephone line. All requests are received by email.

We act without undue delay and reply **within 10 days** as required by the Korean Personal Information Protection Act. If a request needs longer, we tell you the reason and the expected schedule first. Exercising your rights is free unless requests are manifestly unfounded or excessive.

Once you request rectification or erasure, we stop using or disclosing the data concerned until the request is completed. A representative acting on your behalf must provide written authority.

**Portability.** We are not currently a business subject to the mandatory transfer-request duty under Article 35-2 of the Korean Act, but if you ask for a copy of your account data and learning history we provide it in a machine-readable format (JSON or CSV).

---

## 10. Security

**Organisational.** An internal management plan, a minimal number of personnel handling personal data, and regular training for them.

**Technical**

- Access control and access logging for systems processing personal data
- Encryption in transit (HTTPS/TLS)
- Token-based access control; refresh tokens are stored only as SHA-256 hashes, rotated on each use, with the device session revoked if a previously used token is replayed
- Voice recordings held in a private bucket under a per-user path, uploadable only through short-lived signed URLs constrained by file size and content type
- Validation of uploaded files for size, extension, format, and ownership
- Log masking that excludes authorisation headers, tokens, secrets, and unnecessary identifying data

**Physical.** Systems holding personal data run in cloud providers' data centres and rely on those providers' physical access controls.

**Breach notification.** Under Article 34 of the Korean Personal Information Protection Act we notify affected users without undue delay, and where the statutory thresholds are met we report to the Personal Information Protection Commission or the Korea Internet & Security Agency **within 72 hours**.

---

## 11. Contact and Data Protection Officer

The Service is run by an individual. Under Article 31(2) of the Korean Personal Information Protection Act, the operator also serves as the Privacy Officer. Every request and enquiry — access, rectification, erasure, suspension of processing — is received and handled at the single address below.

**Privacy Officer and requests desk**

- Name LEE SANG BYEOK (이상벽)
- Title Operator
- Email **maplebyeok@gmail.com**

Because the Service is run by one person, we do not operate a telephone line or fixed office hours. We reply **within 10 days** of receiving a request. If a request needs longer, we tell you the reason and the expected schedule first.

---

## 12. Regional Rights

### European Economic Area and United Kingdom

**The Service is not distributed in the European Economic Area or the United Kingdom and is not directed at users in those territories.** Those countries are excluded from the app stores' distribution list. We therefore do not appoint an EU Representative under Article 27 GDPR.

If we distribute there in future, we will revise this Policy before doing so to publish the legal bases, data subject rights, supervisory authority complaint route, and EU Representative details that the GDPR requires.

### Republic of Korea

You may seek help from the Personal Information Dispute Mediation Committee (1833-6972, www.kopico.go.kr), the Privacy Infringement Report Centre (118, privacy.kisa.or.kr), the Supreme Prosecutors' Office (1301), or the National Police Agency (182).

### Elsewhere

Where the law of your country grants further rights, we honour them. Contact us using Section 11.

---

## 13. Children

The Service is for users aged **16 and over**, and we do not permit registration below that age. We do not knowingly collect personal data from anyone under 16.

The Korean Personal Information Protection Act requires a legal guardian's consent to collect data from children under 14. As a one-person service we have no way to verify guardian consent, so we set a single, stricter joining age of **16** and do not accept registrations below it.

If we learn that we hold data from someone under 16 we delete it without undue delay. A holder of parental responsibility may contact maplebyeok@gmail.com to request deletion.

---

## 14. Transparency of AI Pronunciation Assessment

Pronunciation scores are produced automatically by a speech recognition and pronunciation assessment engine.

- **Logic**: your recording is compared with the reference pronunciation of the target sentence (a normalised string derived by applying Korean phonological rules), and accuracy is calculated at sentence and syllable level.
- **Process**: choose a sentence and record → upload to private storage → the engine compares and scores → results returned → the original recording is deleted.
- **Method**: we use the pronunciation assessment feature of an external speech service. **We do not use your recordings to train artificial intelligence models.**
- **Status**: scores are study feedback and produce no legal or similarly significant effects, so they are not a decision under Article 37-2 of the Korean Personal Information Protection Act.

If you disagree with a result, contact maplebyeok@gmail.com for an explanation and human review.

---

## 15. Changes to This Policy

We announce changes in the app at least **7 days** before they take effect, and at least **30 days** before for changes that disadvantage users.

**Revision history**

- 2026-09-04 — Changed advertising privacy choice paths and the items available in app settings
- 2026-08-07 — Initial version

---

This Privacy Policy takes effect on 2026-09-04.

[한국어](./privacy-policy.ko.md) · [Terms of Service](./terms-of-service.en.md)
