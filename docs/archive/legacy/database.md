...
현재 백엔드는 Flyway를 사용한다. `spring.jpa.hibernate.ddl-auto`는 기본값 `none`을 유지하고, 실제 스키마 변경은 `backend/src/main/resources/db/migration` 아래 SQL migration으로 관리한다.

초기 baseline은 현재 JPA 엔티티와 맞춘다.

- `V1__baseline_schema.sql`: `users`, `syllables`, `evaluation_log`, `evaluation_syllable`
- `V2__recommended_sentences.sql`: Home 추천 문장용 `recommended_sentences`와 MVP seed 20개 이상
- `V3__extend_evaluation_persistence_model.sql`: Phase 6.1 평가 결과 저장을 위한 `evaluation_log`, `evaluation_syllable` 확장
- 향후 제품 모델인 `practice_session`, `sentence`, `course`, quota 관련 테이블은 별도 migration으로 추가한다.
- 현재 코드의 PK 이름(`user_idx`, `evaluation_log_idx`)은 baseline에서 유지한다. 제품 설계상 권장 이름(`user_id`, `practice_session_id`)으로 바꾸는 작업은 별도 마이그레이션으로 다룬다.
...
Phase 4 구현은 장기 설계의 `sentence`/`sentence_translation` 정규화 전에 소규모 MVP용 `recommended_sentences` 단일 테이블을 먼저 사용한다. 이 테이블은 `original_text`, `standard_pronunciation`, `translation`, `level_label`, `category_code`, `category_label`, `learning_point`, `active`, `sort_order`를 저장하고 `GET /api/sentences/recommended`와 `GET /api/sentences/{sentenceId}`의 데이터 원천이다.
...
- Phase 6.1 구현은 아직 테이블명을 `practice_session`으로 바꾸지 않고 기존 `evaluation_log`를 확장한다. 현재 추가된 컬럼은 `source`, `sentence_id`, `standard_pronunciation`, `recognized_text`, `accuracy_score`, `fluency_score`, `completeness_score`, `pronunciation_score`, `audio_url`이다.
...
- Phase 6.1 구현은 아직 테이블명을 `practice_character_result`로 바꾸지 않고 기존 `evaluation_syllable`을 확장한다. 현재 추가된 컬럼은 `position_no`, `feedback`, `mouth_guide_url`, `tongue_guide_url`이다.
