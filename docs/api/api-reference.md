...
`mouthGuideUrl`과 `tongueGuideUrl`은 제공된 매체 URL입니다. 문장 준비 응답은 입·혀가 같은 시점을 가리키도록 중성의 최종 목표 자세 PNG를 반환합니다. 평가가 완료되면 초성·중성·종성 역할과 복합 모음 구성 요소를 시간순으로 펼쳐 입·혀 segment 수를 맞춘 MP4를 Worker가 생성하고, 앱은 영상 확장자를 영상으로 재생합니다. 단일 프레임이거나 영상 생성에 실패하면 첫 PNG를 정적 fallback으로 표시합니다. 동일 음절·가이드 종류·프레임·매핑 버전 조합의 MP4만 결정적 S3 key와 DB에서 재사용합니다.
