...
- 일일 쿼터 소비 서비스는 있으나 평가 업로드 호출과 연결되지 않음
- 가이드 생성 작업은 비동기로 동작하지만 서버 메모리에만 상태 저장
- Refresh Token은 발급되지만 모바일 자동 갱신 흐름은 완성되지 않음
...
| 일일 쿼터 | Asia/Seoul 기준 하루 동안 사용할 수 있는 연습 횟수 |
| 보상 횟수 | 무료 횟수 소진 후 사용할 수 있는 추가 연습 횟수 |
...
- 기본 일일 무료 연습 횟수는 5회입니다.
- 일일 쿼터는 `Asia/Seoul` 자정에 날짜 기준으로 갱신됩니다.
