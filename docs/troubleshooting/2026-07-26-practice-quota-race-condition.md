...
## 2026-08-03 시간 충전 정책 확장

- 기존 조건부 예약·확정·복구 원자성은 유지하면서 사용자별 최신 물리 행을 현재 평가 에너지로 사용합니다.
- 최초 자연 충전 횟수 예약은 `next_refill_at`이 없을 때만 1시간 timer를 시작하므로 추가 소모나 향후 광고 보상이 timer를 초기화하지 않습니다.
- 조회·예약 전에 완료된 1시간 구간 수만큼 충전하고 최대 5회에서 timer를 제거합니다. 날짜 변경은 더 이상 초기화 조건이 아닙니다.
- 예약 실패 복구로 다시 5회가 되면 timer를 제거하며, 동시 예약 10개와 동일 예약 확정·복구 회귀 테스트를 계속 적용합니다.
- V14가 nullable `next_refill_at`을 추가하므로 실제 MySQL 연속 migration과 장시간 실행 중 예약의 배포 경계는 운영 확인이 필요합니다.
...
원자 `UPDATE` repository 메서드와 최초 생성용 lock 조회를 제거하고 이전 엔티티 상태 전이 구현으로 되돌립니다. 시간 충전까지 롤백한다면 V14의 `next_refill_at`은 nullable이므로 구버전 코드가 무시할 수 있지만, 자정 초기화로 정책이 되돌아가므로 배포 전 사용자 고지가 필요합니다. 원자 전이 자체를 롤백하면 동시 초과 예약 위험이 다시 발생하므로 운영 트래픽을 차단하거나 사용자별 요청을 외부에서 직렬화해야 합니다.
