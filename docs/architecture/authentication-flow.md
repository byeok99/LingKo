...
sequenceDiagram
    ...
    U->>A: 로그인 수단 선택
    A->>B: GET /api/legal/consent/policy (인증 없음)
    B-->>A: 현재 documentVersion
...
동의는 **계정이 만들어지기 전에** 받습니다. 계정 생성 후에 받으면 거부한 사용자의 개인정보가 이미 서버에 생긴 상태가 되어 즉시 삭제하는 경로를 따로 만들어야 합니다. 화면을 열기 전 공개 정책 endpoint에서 현재 버전을 받아 앱·서버의 배포 시점 차이를 제거합니다. 이 조회가 실패하면 인증을 시작하지 않는 fail-closed 방식입니다. 기록 자체는 사용자에게 귀속되어야 하므로 로그인 성공 직후에 전송합니다.
...
`GET /legal/{document}`와 `GET /api/legal/consent/policy`는 예외로 인증을 요구하지 않습니다. 전자는 아직 계정이 없는 가입 화면 사용자와 스토어 심사자가 문서를 열기 위해 필요하고, 후자는 가입 화면이 현재 문서 버전을 알기 위해 필요합니다. 두 응답 모두 사용자 데이터를 포함하지 않습니다.

## 모바일 세션

- 세션은 `flutter_secure_storage`에 저장합니다.
- 신규 로그인은 `GET /api/legal/consent/policy`를 먼저 호출해 서버의 현재 버전으로 동의 화면을 구성합니다. 조회 실패 시 인증을 시작하지 않습니다.
...
- 동의 POST가 실패해도 후속 상태 조회가 `required=false`이면 응답만 유실된 것으로 보고 Home으로 진행합니다. 서버 버전이 바뀌어 `required=true`이면 이전 체크를 초기화하고 새 문서를 다시 확인하게 합니다.
