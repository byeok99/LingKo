...
현재 변환기는 대표 받침, 연음, 겹받침 분리, 비음화, 유음화, 경음화, 구개음화, `ㅎ` 관련 규칙과 자음 뒤 `ㅢ`를 처리합니다. 형태소 분석기나 예외 발음 사전은 포함하지 않으므로 어휘·문법 정보가 필요한 모든 표준 발음 예외를 완전하게 보장하지는 않습니다. 지원 규칙은 회귀 corpus로 고정하고 새 예외는 검증된 발음 근거와 함께 확장합니다.
...
sequenceDiagram
    ...
    W->>W: 초성·중성·종성 역할별 동기화 프레임 구성
    W->>D: 현재 매핑 버전의 음절·유형별 MP4 조회
    W->>S: 저장 미디어가 없으면 버전별 결정적 MP4 cache 조회
...
가이드 영상은 문장 준비 단계가 아니라 평가 완료 결과를 조립할 때 모든 음절에 생성합니다. 글자 점수가 없더라도 Result에서 각 음절 가이드를 열 수 있으므로 점수가 아닌 프레임 전환 유무로 영상 여부를 결정합니다. 초성·중성·종성 역할을 구분하고 복합 모음은 반모음에서 목표 모음으로 이어지는 기존 이미지 배열로 펼칩니다. 한 트랙에 자음 이미지가 없으면 해당 구간을 삭제하지 않고 인접 자세를 유지해 입·혀 segment 수를 같게 합니다. 초성 `ㅎ`에는 잘못된 구강 위치를 부여하지 않고 다음 모음 자세를 유지합니다.

DB의 음절·유형별 저장 MP4는 현재 매핑 버전(`phonology-v3`)과 일치할 때만 조회합니다. 연속된 동일 자세는 정적 PNG로 축약하고, 서로 다른 음절이라도 가이드 종류·프레임 순서·버전이 같으면 하나의 결정적 S3 key를 공유합니다. 개별 이미지 전이 clip도 별도 cache하므로 완성 영상 cache miss에서는 필요한 clip만 생성하고 나머지는 FFmpeg로 병합합니다. 같은 프로세스의 동시 최초 요청은 key별로 직렬화합니다. 외부 생성 실패는 기존 PNG로 fallback하므로 평가 결과 자체를 잃지 않습니다. 최초 전이 cache miss에는 Replicate polling과 FFmpeg 병합 시간이 포함될 수 있어 기본 Worker lease와 앱 polling 범위는 600초로 맞춥니다.
