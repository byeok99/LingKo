...
초성·중성·종성 역할과 복합 모음의 구성 요소를 기존 이미지의 시간순 프레임으로 해석합니다. 특정 부위 이미지가 없는 자음 구간은 가장 가까운 실제 자세를 유지해 입·혀 트랙 길이를 맞춥니다. 저장된 음절·전이 유형의 MP4는 현재 매핑 버전과 일치할 때만 DB에서 재사용하고, 프레임·버전을 포함한 결정적 S3 키를 조회합니다. 캐시가 없으면 Replicate와 FFmpeg로 생성하며 실패 시 이미지 가이드가 대안이 됩니다.
