...
- **시각적 가이드**: 한글 음절을 초성·중성·종성 역할로 분해하고 기존 입·혀 이미지를 시간순 전이 영상으로 조합합니다. 이는 주요 조음 위치를 보여주는 학습 참고 자료이며 실제 발성 기관의 움직임을 정밀 시뮬레이션하지 않습니다.
...
| 반복되는 가이드 영상 생성 비용 | 매핑 버전을 검증하는 DB 미디어 조회와 결정적 S3 캐시 키 |
