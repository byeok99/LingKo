...
공통 정책이 바뀌면 `AGENTS.md`를 고친다. Claude 전용 동작만 이 파일에 남긴다. 두 파일을 모두 수정하면 `docs/private/work-logs/WORK_LOG.md`에 기록한다.
...
- 수정 대상 폴더가 정해지면 **파일을 고치기 전에** `docs/private/work-logs/` 아래 그 폴더에 대응하는 `WORK_LOG.md`를 먼저 읽는다.
...
app/          Flutter 클라이언트 (Dart, Material 3)
  lib/api/        서버 REST client (auth, evaluation, sentence, quota, legal)
...
docs/         api / architecture(+adr) / data / development /
              overview / security / legal / engineering
  private/        Git 제외 개인 운영 문서·과거 자료·작업 이력
```
...
- 광고 보상은 서버 SSV 검증 후 지급하며 최대 보유량 5회를 지킨다. 자연 충전 timer는 변경하지 않는다.
